...
La mejor forma para aplicar una memoria corta es sin duda usando MemorySaver, pero existen otras alternativas, como almacenar el histórico en una variable e ir pasando esa variable una y otra vez para que el LLM pueda tener el contexto de la conversación.

Vemos un ejemplo sencillo para poder comprender cómo hacerlo:

```python  hl_lines="2 4 5 7"
# Almacenamos las preguntas t respuestas en una variable.
...
Como podemos observar, esta forma también tendría memoria, aunque se tendría que manejar de forma manual.
Hemos de tener en cuenta, que podríamos almacenar esa información en una variable, una base de datos o un archivo, aunque más adelante veremos mejores formas de hacerlo, esta puede ser una posible solución a tu proyecto.
