...
La **memoria** es esencial para que el grafo recuerde información a lo largo de múltiples interacciones, lo que permite que el sistema sea más conversacional y útil.  

Cuando **no hay memoria**, cada vez que invocamos el grafo, se reinicia desde cero, sin recordar interacciones anteriores.  
Cuando **activamos la memoria**, el grafo guarda el historial de mensajes, permitiendo respuestas basadas en el contexto de toda la conversación.  
...
En el próximo tema, aprenderemos sobre **Chains y Flujos de Trabajo**, donde veremos cómo conectar múltiples nodos en secuencia y crear pipelines más avanzados.
